...
from colima_helper.gui.events import LogReceivedEvent, ContainerCreatedEvent
from colima_helper.gui.services.log_collector import LogsCollector
...
class DockerLogsCollectorService(LogsCollector):
    ...
    def container_created_event_listener(self, event: ContainerCreatedEvent):
        if 'colima-helper-log' not in event.container['Labels']:
            return
        if json.loads(event.container['Labels']['colima-helper-log']):
